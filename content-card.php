...
printf( '<div class="grid-row%s clearfix">',
	get_option( 'pls_enable_masonry', false ) ? ' masonry-enabled' : ''
);
    ...
    while ( have_posts()) :
		...
		?>
        <article <?php post_class( 'newsplus entry-grid entry-card col'. $col . $fclass . $lclass . $ad_class ); printf( '%s', $schema['container'] ); ?>>
            <div class="card-wrap">
                <?php
                if ( $ad_class ) {
