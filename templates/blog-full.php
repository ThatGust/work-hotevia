<?php
/**
* Template Name: Blog - Full Post Style
*
* Description: A full post style blog template.
*/
	...
	<div class="primary-row">
		...
		if ( $custom_query->have_posts() ) {
			...
			while ( $custom_query->have_posts() ) :
				...
				<article<?php printf( '%s', $schema['container'] ); ?> id="post-<?php the_ID(); ?>" <?php post_class( 'newsplus entry-classic entry-full clearfix' . $ad_class ); ?>>				
					<?php
					if ( $ad_class ) {
                    ...
                    <h2<?php printf( '%s', $schema['heading'] ); ?> class="entry-title"><a href="<?php the_permalink(); ?>" title="<?php the_title(); ?>"><?php the_title(); ?></a></h2>
                    
                    <?php printf( '%s', $rows['row_4'] );
                    if ( 'true' !== get_option( 'pls_hide_blog_full_images' ) ) {
                        get_template_part( 'formats/format', get_post_format() );
                    }
                    
					?>
					<div class="entry-content articlebody"<?php echo (  get_option( 'pls_schema', false ) ? ' itemprop="articleBody"' : '' ); ?>>        
						<?php
