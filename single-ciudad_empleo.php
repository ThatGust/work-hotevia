...
$numero_de_empleos_raw = get_field('baneres_ciudades', 'option');
$numero_de_empleos = intval($numero_de_empleos_raw);
if ($numero_de_empleos < 1) {
    $numero_de_empleos = 10;
}

$banners_repetibles = array();
if (!empty($banners_de_contenido) && is_array($banners_de_contenido)) {
    foreach ($banners_de_contenido as $indice_banner => $banner_item) {
        if ($indice_banner === 0) {
            continue;
        }
        if (!empty($banner_item['html'])) {
            $banners_repetibles[] = $banner_item;
        }
    }
}

$posts_per_page_raw = get_field('num_ofer_ciudad', 'option');
$posts_per_page = intval($posts_per_page_raw);
if ($posts_per_page < 1) {
    $posts_per_page = 20;
}
                                ...
                                <div class="ciudad-bloque ciudad-listado job-offers">
                                    ...
                                    <?php if (!empty($rows)): ?>
                                        ...
                                        <?php foreach ($rows as $o_row): ?>
                                            ...
                                            <?php if (!empty($banners_repetibles) && ($contador_empleos % $numero_de_empleos === 0)): ?>
                                                <?php $banner_index = (intval($contador_empleos / $numero_de_empleos) - 1) % count($banners_repetibles); ?>
                                                <?php if (isset($banners_repetibles[$banner_index]['html']) && !empty($banners_repetibles[$banner_index]['html'])): ?>
                                                    <div class="banner-contenido" style="margin-top:20px; margin-bottom:20px;">
                                                        <?php echo $banners_repetibles[$banner_index]['html']; ?>
                                                    </div>
                                                <?php endif; ?>
                                            <?php endif; ?>
                                        <?php endforeach; ?>
